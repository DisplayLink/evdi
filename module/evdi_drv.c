...
static void evdi_add_device(void)
{
  ...
  evdi_context.devices[evdi_context.dev_count] = platform_device_register_full(&pdevinfo);
  if (0 != dma_set_mask(&evdi_context.devices[evdi_context.dev_count]->dev, DMA_BIT_MASK(64))) {
    EVDI_DEBUG("Unable to change dma mask to 64 bit. Sticking with 32 bit.\n");
  }
...
static ssize_t loglevel_show(struct device *dev, struct device_attribute *a, char *buf) {
  return snprintf(buf, PAGE_SIZE, "%u\n", g_evdi_loglevel);
}
                              ...
                              size_t count)
{
    ...
    parsed = sscanf(buf, "%u", &val);
    if (parsed) {
        if (val >= EVDI_LOGLEVEL_ALWAYS && val <= EVDI_LOGLEVEL_VERBOSE) {
...
static int __init evdi_init(void)
{
  ...
  if (!PTR_RET(evdi_context.root_dev)) {
    for (i = 0; i < ARRAY_SIZE(evdi_device_attributes); i++) {
        device_create_file(evdi_context.root_dev, &evdi_device_attributes[i]);
    }
  }
...
static void __exit evdi_exit(void)
{
  ...
  if (!PTR_RET(evdi_context.root_dev)) {
    for (i = 0; i < ARRAY_SIZE(evdi_device_attributes); i++) {
      device_remove_file(evdi_context.root_dev, &evdi_device_attributes[i]);
    }
